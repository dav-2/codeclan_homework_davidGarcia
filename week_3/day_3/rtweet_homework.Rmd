...
```{r}
ggplot(code_clan_tweets, aes(x = "code_clan_tweets", y = favorite_count)) +
    geom_boxplot()
```
...
total_number_favorited_tweets = if(is_quote == F) 
```{r}
code_clan_tweets %>% 
  summarise(total_number_favorited_tweets = if(is_quote == F){sum(favorite_count)})
```
...
code_clan_tweets %>% 
  group_by(source) %>% 
  summarise(mean_retweets = if(is_quote == F){mean(retweet_count)})
```
...
code_clan_tweets %>% 
  group_by(media_type) %>% 
  summarise(total_tweets_by_media = sum(favorite_count)) %>% 
  arrange(desc(total_tweets_by_media)) %>% 
  mutate(media_type = replace(media_type, 
                                    is.na(media_type), 
                                    "text"))
```
...
```{r}
code_clan_tweets %>% 
  summarise(mean_number_characters = mean(str_count(text)))
```
...
```
```{r}
code_clan_tweets_joined <- left_join(code_clan_tweets, code_clan_info, "tweet_id")
code_clan_tweets_joined
```
...
codeclan_hashtags <- code_clan_tweets_joined %>% 
  ...
  mutate(hashtags_true = str_detect(hashtags, pattern)) %>% 
  filter(hashtags_true == TRUE) %>% 
  select(tweet_id, hashtags)
codeclan_hashtags
```
